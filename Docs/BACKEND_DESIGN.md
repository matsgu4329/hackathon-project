...
> **구현 노트 (Phase 5)**: Phase 4(WeatherSnapshot)보다 먼저 구현되어, 날씨 조회를 `TodayWeatherProvider` 인터페이스(`com.skinclock.recommendation`)로 분리하고 `MockTodayWeatherProvider`(CLEAR, UV 7 고정)로 우선 구현했습니다. Phase 4 완료 후에는 이 인터페이스의 구현체만 `WeatherSnapshotRepository` 기반으로 교체하면 되고, `RecommendationService`의 나머지 로직은 그대로 둡니다.
